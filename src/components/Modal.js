  ...
  const handleDonate = async () => {
    ...
    try {
      const response = await fetch(
        "https://donate-urgent-backend.onrender.com/api/create-checkout-session",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    <div className="modal-container" onClick={onClose}>
      <div className="modal-Box" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="amounts">
          <button className="amt-btn" onClick={() => handlePresetClick("25")}>$25</button>
          <button className="amt-btn" onClick={() => handlePresetClick("35")}>$35</button>
          <button className="amt-btn" onClick={() => handlePresetClick("50")}>$50</button>
          <button className="amt-btn" onClick={() => handlePresetClick("60")}>$60</button>
          <button className="amt-btn" onClick={() => handlePresetClick("75")}>$75</button>
          <button className="amt-btn" onClick={handleButtonClick}>Custom</button>
        </div>

        <p className="note">
          Amounts shown are suggestions. Donations are voluntary.
        </p>
